...
def generate_if_else_strings(language_code, indentation=""):
    ...
    if ADD_MY_APP:
        ifelse += indentation + "else if (infCMDType == ASR_CMD_MY_APP)\n" +\
            indentation + "    retString = cmd_my_app_"+ language_code +";\n"
...
def generate_if_else_number(indentation=""):
    ...
    if ADD_MY_APP:
        ifelse += indentation + "else if (infCMDType == ASR_CMD_MY_APP)\n" +\
            indentation + "    cmd_number = NUMBER_OF_MY_APP_CMDS;\n"
...
def generate_main_file():
    ...
    with open(main_file, "a", encoding='utf-8') as header_file:
        ...
        if ADD_MY_APP:
            header_file.writelines("#define NUMBER_OF_MY_APP_CMDS       " + str(len(cmd_my_app[language_codes[0]][COMMANDS_TRIMMED_LIST])) + "\n")
...
#            write_file(header_file, language_code, "*ww_", ww[language_code][FILE_DATA], "static ")
            ...
            if ADD_MY_APP:
                write_file(header_file, language_code, "*cmd_my_app_", cmd_my_app[language_code][FILE_DATA_TRIMMED], "static ")
...
def generate_files():
    ...
    for language_code in language_codes:
        ...
        if ADD_MY_APP:
            cmd_my_app[language_code] = generate_file("../oob_demo_", language_code, "*cmd_my_app_", "/CMD_MY_APP.xml")
        ...
        if ADD_MY_APP:
            print("My App Commands: \n\t", cmd_my_app[language_code])
