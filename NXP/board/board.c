...
void BOARD_SAI_Init(sai_init_handle_t saiInitHandle)
{
    ...
    /* SAI init */
    SAI_Init(BOARD_AMP_SAI);
    SAI_GetClassicI2SConfig(&config, kSAI_WordWidth16bits, kSAI_MonoLeft, kSAI_Channel0Mask);

#if USE_ALEXA_SOUND_PROMPT || USE_TFA || USE_MQS
    ...
    saiAmpFormat.sampleRate_Hz = kSAI_SampleRate16KHz;
    SAI_GetClassicI2SConfig(&config, kSAI_WordWidth32bits, kSAI_MonoLeft, kSAI_Channel0Mask);
#endif
    ...
    SAI_TransferTxSetConfigEDMA(BOARD_AMP_SAI, saiInitHandle.amp_sai_tx_handle, &config);
    config.syncMode = kSAI_ModeSync;
    SAI_TransferRxSetConfigEDMA(BOARD_AMP_SAI, saiInitHandle.amp_sai_rx_handle, &config);
    ...
    NVIC_SetPriority(BOARD_AMP_SAI_RX_IRQ, configMAX_SYSCALL_INTERRUPT_PRIORITY - 1);
